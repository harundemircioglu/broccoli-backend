...
public abstract class Repository<TEntity, TId> : IRepository<TEntity, TId> where TEntity : class
{
    ...
    public async Task<IList<TEntity>> GetByUserIdAsync(int userId)
    {
        return await PostgresContext.Set<TEntity>()
            .Where(e => (int)e.GetType().GetProperty("UserId").GetValue(e) == userId)
            .ToListAsync();
    }
